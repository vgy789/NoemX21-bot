...
- Если фейс-контроль или ручная уборка удаляет уже вошедшего участника, бот выполняет kick через `banChatMember` + немедленный `unbanChatMember(only_if_banned=true)`, поэтому пользователь не остаётся в чёрном списке группы.
