...
### Legacy member-tag import

Основной production-интерфейс не требует доступа к серверу:

1. Сделать backup PostgreSQL.
2. Применить migration обычным release-процессом.
3. Владельцу открыть в личном чате с ботом `Настройки → Импорт тегов`.
4. Отправить JSON до 2 МиБ и сверить агрегированный отчёт. Импорт применяется сразу.
5. Наблюдать только aggregate queue depth, retry count и Telegram error type; не выводить ID, login или tag.

Приложение держит исходный JSON только в памяти на время запроса и удаляет сообщение с
документом после обработки. Telegram остаётся внешним обработчиком документа по своим
правилам хранения. CLI-флаги `-dry-run` и `-apply` сохраняются только как аварийный
операторский способ.

Для текущего `.json` dry-run должен показать `rows=4062`, `accepted=4046`,
`conflict_ids=7`, `skipped_null_status=5`. Development-агент production-импорт не запускает.
