	...
	t.Run("success with rocketchat platform", func(t *testing.T) {
		...
		mockRepo.EXPECT().
			...
			Return(db.GetStudentProfileRow{
				...
				Level:         pgtype.Int4{Int32: 10, Valid: true},
				ExpValue:      pgtype.Int4{Int32: 5000, Valid: true},
				Prp:           pgtype.Int4{Int32: 8, Valid: true},
				Crp:           pgtype.Int4{Int32: 4, Valid: true},
				Coins:         pgtype.Int4{Int32: 200, Valid: true},
			}, nil)

		profile, err := svc.GetProfileByExternalID(ctx, db.EnumPlatformRocketchat, "rc-id-123")
		assert.NoError(t, err)
		assert.NotNil(t, profile)
		assert.Equal(t, "student-login", profile.Login)
		assert.Equal(t, "Moscow", profile.CampusName)
		assert.Equal(t, int32(10), profile.Level)
	})
}
