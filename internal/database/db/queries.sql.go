...
const resetTelegramGroupSettingsByChatID = `-- name: ResetTelegramGroupSettingsByChatID :one
WITH reset_group AS (
    UPDATE telegram_groups
...
type ResetTelegramGroupSettingsByChatIDParams struct {
	ChatID                int64  `json:"chat_id"`
	OwnerTelegramUserID   int64  `json:"owner_telegram_user_id"`
	OwnerTelegramUsername string `json:"owner_telegram_username"`
}

func (q *Queries) ResetTelegramGroupSettingsByChatID(ctx context.Context, arg ResetTelegramGroupSettingsByChatIDParams) (int64, error) {
	row := q.db.QueryRow(ctx, resetTelegramGroupSettingsByChatID, arg.ChatID, arg.OwnerTelegramUserID, arg.OwnerTelegramUsername)
	var column_1 int64
	err := row.Scan(&column_1)
	return column_1, err
}
