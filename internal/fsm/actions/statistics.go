...
func init() {
	Register(NewBasicPlugin("statistics", func(registry *fsm.LogicRegistry, deps *Dependencies) {
		...
		registry.Register("get_user_stats", func(ctx context.Context, userID int64, payload map[string]interface{}) (string, map[string]interface{}, error) {
			...
			acc, err := deps.Queries.GetUserAccountByExternalId(ctx, db.GetUserAccountByExternalIdParams{
				Platform:   db.EnumPlatformTelegram,
				ExternalID: fmt.Sprintf("%d", userID),
			})
			if err != nil {
				deps.Log.Error("failed to get user account", "user_id", userID, "error", err)
				return "", nil, err
			}

			// 2. Get profile
			profile, err := deps.Queries.GetStudentProfile(ctx, acc.StudentID)
			if err != nil {
				deps.Log.Error("failed to get student profile", "login", acc.StudentID, "error", err)
				return "", nil, err
			}
		...
		registry.Register("get_peer_data_with_permissions", func(ctx context.Context, userID int64, payload map[string]interface{}) (string, map[string]interface{}, error) {
			...
			// 1. Get peer profile
			profile, err := deps.Queries.GetPeerProfile(ctx, login)
			if err != nil {
				// Handle not found
			...
			acc, err := deps.Queries.GetUserAccountByStudentId(ctx, login)
			if err == nil {
		...
		registry.Register("get_user_skills", func(ctx context.Context, userID int64, payload map[string]interface{}) (string, map[string]interface{}, error) {
			...
			skills, err := deps.Queries.GetStudentSkills(ctx, acc.StudentID)
			if err != nil {
				return "", nil, err
			}
		...
		registry.Register("generate_radar_chart", func(ctx context.Context, userID int64, payload map[string]interface{}) (string, map[string]interface{}, error) {
			...
			for _, uRaw := range usersRaw {
				...
				skills, _ := deps.Queries.GetStudentSkills(ctx, acc.StudentID)
				for _, s := range skills {
