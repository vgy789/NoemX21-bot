...
func Register(
	...
	registry.Register("load_sapphire_giveaway_progress", func(ctx context.Context, userID int64, payload map[string]any) (string, map[string]any, error) {
		...
		updates["giveaway_progress_text"] = formatProgress(rows)
		return "", updates, nil
	})
...
func formatProgress(rows []db.SapphireGiveawayParticipant) string {
	if len(rows) == 0 {
		return "Пока нет участников."
	}
	var b strings.Builder
	for i, row := range rows {
		fmt.Fprintf(&b, "%d. `%s` — %d\n", i+1, row.S21Login, row.CountedProjectsCount)
	}
